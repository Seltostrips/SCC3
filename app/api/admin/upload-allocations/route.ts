...
export async function POST(request: Request) {
  try {
    ...
    // Frontend will now send it as 'file' instead of 'csvFile'
    ...
    for (const record of records) {
      if (!ALLOWED_ASSESSMENT_YEARS.includes(record.AssessmentYear)) {
        return NextResponse.json({ message: `Invalid Year: ${record.AssessmentYear}` }, { status: 400 });
      }
    ...
    await prisma.$transaction(
      allocationsToCreate.map((allocation) =>
        prisma.allocation.upsert({
          where: { clientPAN_assessmentYear: { clientPAN: allocation.clientPAN, assessmentYear: allocation.assessmentYear } },
          update: allocation,
          create: allocation,
        })
      )
