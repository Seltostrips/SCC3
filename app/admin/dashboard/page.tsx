...
export default function AdminDashboard() {
  ...
  // FULL STATUS CONTROLLER FOR ADMIN (Fixes the Filed Bug)
  ...
  const handleViewFiles = async (allocationId: string) => {
    if (expandedDocs[allocationId]) {
      const newDocs = { ...expandedDocs }; delete newDocs[allocationId]; setExpandedDocs(newDocs); return;
    }
    try {
      const res = await fetch(`/api/admin/allocation-files?id=${allocationId}`);
      setExpandedDocs(prev => ({ ...prev, [allocationId]: (await res.json()).files || [] }));
    } catch (err) {}
  };
  ...
  const handleAllocationUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setAllocationMessage("Upserted successfully."); setAllocationFile(null); fetchAllocations(); } 
      else setAllocationMessage((await res.json()).message);
